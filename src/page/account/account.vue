      ...
      <ul class="p_zhleft">
        <li id="list_account">
          ...
          <ul class="p_zhminav">
            ...
            <router-link to="/account/home/recharge" tag="li" @click.native="navEvent('充值')" active-class="clicked" exact><a>充值</a></router-link>
            <router-link to="/account/home/withdrawal" tag="li" @click.native="navEvent('提现')" active-class="clicked" exact><a>提现</a></router-link>
...
export default {
  ...
  beforeRouteUpdate(to, from, next){
    // 跳转到充值前的判断，暂时未添加任何判断
    if (to.path.indexOf("home/recharge") > -1) {
      next()
    } else {
      next()
    }
  ...
  methods: {
    ...
    //       if(document.getElementById(link[i].type)){
