<template>
  <div class="p_jiluword s_tab_con">
      ...
      <table border="0" cellpadding="0" cellspacing="0" width="100%" class="p_jilutable p_wordmore">
        <tbody>
          ...
          <tr v-for="(item, index) in list.page">
            ...
            <td>{{item.amount}}</td>
            <td>{{item.ableAmount}}</td>
            <td>{{item.provideDate}}</td>
...
export default {
        ...
        page: [
          {
            amount: '300',
            ableAmount: '280',
            provideDate: '2017-01-31',
            expireDate: '2018-07-31',
            status: '可使用',
            remark: '',
          },
          {
  ...
  mounted (){
    this.requestData();
    this.$emit('showPaper', this.list.ableAmount);
  },
  methods: {
    toggle (index, view) {
